...
# 客戶標籤後的完整姓名（2~3 字，不含機構／職稱字樣）。
# 「保戶」「要保人」這類標籤本身即為強訊號，不需再要求姓氏在字庫內——
# 姓氏字庫永遠列不全（如「陽曼蘭」的陽），靠標籤才能根治這類漏遮。
_NAME_WHOLE = "(?:(?!" + "|".join(_ORG_WORDS) + ")[一-鿿]){2,3}"

# 姓名標籤分兩組：客戶端（要遮）與內部人員（不遮）。
# _NAME_LABELS 由兩者合併而成，只負責回答「這個位置有姓名嗎」；
# 至於要不要遮，由 is_customer_context() / is_agent_context() 逐處判斷。
# 內部人員標籤必須也在偵測清單內，加上 --mask-agent-names 時才能一併遮回來。

# 客戶端標籤 —— 其後的姓名為客戶個資，一律遮罩
_CUSTOMER_LABELS = (
    "姓名", "收件人", "申請人", "當事人", "聯絡人", "負責人", "受文者",
    "立書人", "立契約書人", "要保人", "被保險人", "主被保人", "副被保人",
    "受益人", "保戶", "客戶", "代理人", "法定代理人", "受款人", "借款人",
    "保證人", "病患", "患者", "員工", "推介者", "輔導者", "繼承人",
    "介紹人", "招攬人", "推薦人", "主管",
)

# 內部人員標籤 —— 業務員／帳號／經手人等，其後的姓名預設不遮罩。
# 判斷以「逐處」為單位：同一人在「要保人」處仍會遮、在「帳號」處則保留，
# 因為決定身分的是該處的標籤。
# 可用 CLI --mask-agent-names／GUI 勾選改回一律遮罩。
_AGENT_LABELS = (
    # 業務員
    "業務員", "招攬業務員", "原業務員", "新業務員", "業務人員", "業務代表",
    "服務人員", "登錄業務員", "所屬業務員", "招攬人員",
    # 帳號
    "帳號", "帳戶名稱", "帳號名稱", "使用者", "登入帳號",
    # 經辦／處理本案的內部人員
    "經手人", "經辦人", "承辦人", "處理者", "送件人", "建立者",
)

_NAME_LABELS = "|".join(_CUSTOMER_LABELS + _AGENT_LABELS)

# 姓名前方多遠內出現標籤即視為該情境
_LABEL_CONTEXT_BEFORE = 14
# 欄位提示需短於此長度才視為「欄位標題」，避免鄰格長文誤判
_LABEL_HINT_MAX_LEN = 20


def _has_label_before(text: str, start: int, hint: str,
                      labels: Sequence[str]) -> bool:
    """姓名「前方」或「短欄位標題」中是否出現指定標籤。

    只看前方：「保戶 陳美玲 … 與 業務員/營業處所 相同」中的陳美玲身分是保戶，
    後方的「業務員」不影響判斷。
    提示只採短標籤（欄位標題、表單標籤）——鄰格若是長篇自由文字，
    其中偶然出現的「業務員」不足以判定本格姓名的身分。
    """
    window = text[max(0, start - _LABEL_CONTEXT_BEFORE):start]
    if any(k in window for k in labels):
        return True
    return any(k in line
               for line in hint.splitlines()
               if len(line) <= _LABEL_HINT_MAX_LEN
               for k in labels)


def is_agent_context(text: str, start: int, hint: str = "") -> bool:
    """該處姓名是否為內部人員（業務員／帳號／經手人…）。"""
    return _has_label_before(text, start, hint, _AGENT_LABELS)


def is_customer_context(text: str, start: int, hint: str = "") -> bool:
    """該處姓名是否明確為客戶端（保戶／要保人／被保險人…）。

    內部人員標籤優先：欄位標題「業務員姓名」同時含「業務員」與「姓名」，
    應判為內部人員。
    """
    if is_agent_context(text, start, hint):
        return False
    return _has_label_before(text, start, hint, _CUSTOMER_LABELS)
...
DETECTORS: List[Detector] = [
    ...
    Detector(
        # 客戶標籤（保戶／要保人／被保險人…）後的姓名：
        # 標籤即強訊號，不要求姓氏在字庫內，避免「陽曼蘭」這類漏遮
        # 標籤與姓名之間必須有分隔（空白或冒號）——真實用法皆如此
        # （「保戶 陽曼蘭」「被保險人：王小明」）。少了這個條件，
        # 「客戶身分證字號」會被讀成「客戶」+「身分證」、
        # 工作表名「客戶名單」會被遮成「客戶○○」
        name="name_customer", label="姓名(客戶欄位)", priority=47,
        pattern=re.compile(
            "(?:" + "|".join(_CUSTOMER_LABELS) + r")[\s:：]+(" + _NAME_WHOLE + ")"
        ),
        validator=_validate_person_name,
        group=1,
    ),
