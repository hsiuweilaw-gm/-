...
# 註：業務員相關標籤已移至 _AGENT_LABELS（預設不遮），故不在此列
_NAME_LABELS = (
    "姓名|收件人|申請人|當事人|聯絡人|負責人|受文者|立書人|立契約書人|要保人|"
    "被保險人|受益人|保戶|經辦人|承辦人|代理人|法定代理人|受款人|借款人|"
    "保證人|客戶|病患|患者|員工|推介者|輔導者|繼承人|介紹人|招攬人|推薦人|主管|"
    "主被保人|副被保人|經手人|送件人"
)

# 業務員／帳號相關標籤：其後的姓名視為「內部人員」而非客戶個資，預設不遮罩
# （查核案件需保留業務員身分才能追查；客戶端姓名仍照遮）
# 可用 CLI --mask-agent-names／GUI 勾選改回一律遮罩
_AGENT_LABELS = (
    "業務員", "帳號", "招攬業務員", "原業務員", "新業務員", "業務人員",
    "服務人員", "登錄業務員", "所屬業務員", "業務代表", "招攬人員",
    "帳戶名稱", "帳號名稱", "使用者", "登入帳號",
)
# 姓名前方多遠內出現上述標籤即視為業務員／帳號情境
_AGENT_CONTEXT_BEFORE = 14
# 欄位提示需短於此長度才視為「欄位標題」，避免鄰格長文誤放行
_AGENT_HINT_MAX_LEN = 20


def is_agent_context(text: str, start: int, hint: str = "") -> bool:
    """判斷 text 中位於 start 的姓名，是否出現在業務員／帳號情境。

    只看「姓名前方」的文字：
      「與 帳號 劉湘妘(N224463647) 之 地址 相同」→ 是（帳號在前）
      「保戶 楊惠萍 的 聯絡資料 與 業務員/營業處所 相同」→ 否（業務員在後，
        該姓名的身分是保戶）
    表格情境則另看欄位提示（如欄位標題為「業務員姓名」）。
    """
    window = text[max(0, start - _AGENT_CONTEXT_BEFORE):start]
    if any(k in window for k in _AGENT_LABELS):
        return True
    # 提示只採「短標籤」形式（欄位標題、表單標籤）。
    # 鄰格若是長篇自由文字，其中偶然出現的「業務員」不足以判定本格姓名的身分，
    # 否則同列一提到業務員，整列客戶姓名都會被誤放行。
    return any(k in line
               for line in hint.splitlines()
               if len(line) <= _AGENT_HINT_MAX_LEN
               for k in _AGENT_LABELS)
